...
public class Character_Mobing : MonoBehaviour
{
    ...
    void FixedUpdate()//地面との当たり判定を用いて、自由落下を作る
    {
        if (TimeProgress==true) {
            if (moveZ || moveX || mouseXbool || mouseYbool)
            {
                moveZ = false;
                moveX = false;
                mouseXbool = false;
                ...
                rg.velocity = transform.rotation * new Vector3(X, Physics.gravity.y, Z) * moveSpeed * Time.deltaTime * 100;
